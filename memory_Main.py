...
# Function to process a single JSON object and extract selected columns
def process_json_object(obj):
    entry = {}
    for prefix, event, value in obj:
        if event == 'end_map' and prefix == 'data.item':
            return entry
        if prefix.endswith('id'):
            entry[prefix.split('.')[-1]] = value
    return entry

# Load JSON data from file using ijson
def load_large_json(filename):
    with open(filename, 'r', encoding='utf-8') as data_file:
        parser = ijson.parse(data_file)
        for prefix, event, value in parser:
            if event == 'start_map' and prefix == 'data.item':
                entry = process_json_object(parser)
                if entry:
                    yield entry

data_generator = load_large_json('artworks_100.json')
...
def add_new_entry():
    ...
    data_generator.append(new_entry)

    # Save the updated data back to the file
    with open("artworks_100.json", "w", encoding="utf-8") as json_file:
        # json.dump(data, json_file, indent=2)
        json.dump(data_generator, json_file, indent=2)
...
while True:
    ...
    if any(keyword in choice for keyword in ["modify", "change", "alter", "edit"]):
        ...
        if data_generator:
            ...
            entry = next((entry for entry in data_generator if entry["id"] == entry_id), None)
            if entry:
